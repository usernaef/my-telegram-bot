...
class GameManager:
    ...
    async def handle_start_game(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id not in self.waiting_players:
            ...
            # اگر تعداد بازیکنان به 8 رسید، بازی را شروع کن
            if len(self.waiting_players) == 8:
...
class Game:
    def __init__(self, players: List[Player]):
        ...
        # نقش‌های بازی
        self.roles = [
            "godfather", "minion", "mafia", "mafia",  # مافیا تیم
            "citizen", "citizen", "citizen", "citizen"  # شهروند تیم
        ]
...
async def start_night_phase(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        # ارسال پیام شروع شب به همه
        night_message = Messages.NIGHT_START.format(self.night_count)
        for player in self.players:
            if player.is_alive:
                try:
                    keyboard = None
                    if player.can_kill():  # فقط برای نقش‌های قاتل
