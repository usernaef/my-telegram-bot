...
🤖 سلام! من ربات ضد اسپم و ضد لینک هستم.

برای استفاده از من:
1️⃣ من را به گروه خود اضافه کنید
2️⃣ من را ادمین کنید
3️⃣ دستور /panel را در گروه ارسال کنید
            ...
            if len(user_messages[chat_id][user_id]) > settings['spam_limit']:
                if not await is_admin(update, user_id):
                    try:
                        ...
                        await update.message.reply_text(f"⚠️ کاربر {update.effective_user.first_name} به دلیل اسپم به مدت 1 دقیقه محدود شد.")
                        user_messages[chat_id][user_id] = []
