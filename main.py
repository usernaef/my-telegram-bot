...
async def handle_move(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # پردازش داده callback
        data = query.data.split("_")
        if len(data) != 4 or data[0] != "move":
            await query.answer("داده نامعتبر!")
            return
            
        # استخراج اطلاعات حرکت
        _, player1_id, player2_id, position = data
        game_id = f"{player1_id}_{player2_id}"
        row = int(position[0])
        col = int(position[1])
        
        logger.info(f"Processing move for game {game_id} at position ({row}, {col})")
        
        if game_id not in games:
            await query.answer("این بازی معتبر نیست!")
            return
