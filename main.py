...
async def check_links_periodically():
    while True:
        await asyncio.sleep(600)  # اول صبر میکنه
        try:
            if target_group_id:
                try:
                    messages = await application.bot.get_chat_history(target_group_id, limit=100)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(domain in update.message.text.lower() 
           for domain in ['http', 'www', '.com', '.ir', 't.me']):
        if not await is_admin(chat_id, update.effective_user.id):
            try:
