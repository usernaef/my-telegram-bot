...
async def next_turn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if elapsed_time.total_seconds() >= game.game_duration:
...
async def accuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ایجاد لیست دکمه‌ها برای انتخاب بازیکن مظنون
    keyboard = []
    row = []
    for player_id, player_name in game.players.items():
        if player_id != user_id:  # بازیکن نمی‌تواند خودش را متهم کند
            ...
            if len(row) == 2:  # دو دکمه در هر ردیف
                keyboard.append(row)
                row = []
    if row:  # اضافه کردن ردیف آخر اگر ناقص است
        keyboard.append(row)
